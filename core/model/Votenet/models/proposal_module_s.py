...
class ProposalModule(nn.Module):
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr, num_proposal, sampling, seed_feat_dim=256, scale_factor=2):
        super().__init__() 
        ...
        self.seed_feat_dim = seed_feat_dim//scale_factor
        ...
        self.vote_aggregation = PointnetSAModuleVotes( 
                ...
                radius=0.3,
                nsample=16,
                mlp=[self.seed_feat_dim, 128//scale_factor, 128//scale_factor, 128//scale_factor],
                use_xyz=True,
                normalize_xyz=True
            )
        ...
        self.conv1 = torch.nn.Conv1d(128//scale_factor,128//scale_factor,1)
        self.conv2 = torch.nn.Conv1d(128//scale_factor,128//scale_factor,1)
        self.conv3 = torch.nn.Conv1d(128//scale_factor,2+3+num_heading_bin*2+num_size_cluster*4+self.num_class,1)
        self.bn1 = torch.nn.BatchNorm1d(128//scale_factor)
        self.bn2 = torch.nn.BatchNorm1d(128//scale_factor)

    def forward(self, xyz, features, end_points, end_points_t=None):
        """
        Args:
            xyz: (B,K,3)
            features: (B,C,K)
        ...
        if self.sampling == 'vote_fps':
            ...
            if end_points_t==None:
                xyz, features, fps_inds = self.vote_aggregation(xyz, features)
            else: # sample as the teacher
                xyz, features, fps_inds = self.vote_aggregation(xyz, features, end_points_t['aggregated_vote_inds'])
